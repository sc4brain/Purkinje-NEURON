...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 CaP /Users/hashmup/nC_projects/PurkinjeCell.ncx/generatedNEURON/x86_64/CaP.mod\n");
...
static int _reset;
static char *modelname = "Channel: CaP";
...
static int  _f_rates ( _threadargsprotocomma_ double _lv ) {
  ...
  _ltemp_adj_m = pow( 3.0 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   _ltemp_adj_h = pow( 3.0 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   _lA_alpha_m = 8.5 ;
   _lk_alpha_m = - 0.08 ;
   _ld_alpha_m = 8.0 ;
   _lalpha = _lA_alpha_m / ( exp ( ( _lv - _ld_alpha_m ) * _lk_alpha_m ) + 1.0 ) ;
   _lA_beta_m = 35.0 ;
   _lk_beta_m = 0.0689655 ;
   _ld_beta_m = - 74.0 ;
   _lbeta = _lA_beta_m / ( exp ( ( _lv - _ld_beta_m ) * _lk_beta_m ) + 1.0 ) ;
   mtau = 1.0 / ( _ltemp_adj_m * ( _lalpha + _lbeta ) ) ;
   minf = _lalpha / ( _lalpha + _lbeta ) ;
   _lA_alpha_h = 0.0015 ;
   _lk_alpha_h = 0.125 ;
   _ld_alpha_h = - 29.0 ;
   _lalpha = _lA_alpha_h / ( exp ( ( _lv - _ld_alpha_h ) * _lk_alpha_h ) + 1.0 ) ;
   _lA_beta_h = 0.0055 ;
   _lk_beta_h = - 0.125 ;
   _ld_beta_h = - 23.0 ;
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   gion = gmax * ( pow( ( 1.0 * m ) , 1.0 ) ) * ( pow( ( 1.0 * h ) , 1.0 ) ) ;
   ica = gion * ( v - eca ) ;
   }
 _current += ica;
...
static void _initlists(){
 ...
 _slist1[1] = &(h) - _p;  _dlist1[1] = &(Dh) - _p;
   ...
   _t_mtau = makevector(251*sizeof(double));
   _t_hinf = makevector(251*sizeof(double));
   _t_htau = makevector(251*sizeof(double));
_first = 0;
}
